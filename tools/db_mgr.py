...
def get_participant_join_timestamp(user_id, campaign_id):
    cur = conn.cursor()
    cur.execute(
        'select join_timestamp from et_campaign_to_user_maps where user_id=%s and campaign_id=%s;',
        (
            user_id,
            campaign_id
        )
...
def bind_participant_to_campaign(user_id, campaign_id):
    ...
    cur.execute(
        'update et_users set campaign_id = %s where id=%s;',
        (
            campaign_id,
            user_id
        )
...
def create_or_update_campaign(campaign_id, creator_id, name, notes, config_json, start_timestamp, end_timestamp):
    cur = conn.cursor()
    # create campaign record
    cur.execute('select exists(select 1 from et_campaigns where id=%s);', (campaign_id,))
    exists = cur.fetchone()[0]
    if exists:
        # update existing if the owner made the request
        cur.execute('select creator_id from et_campaigns where id=%s;', (campaign_id,))
        if creator_id == cur.fetchone()[0]:
            cur.execute(
                'update et_campaigns set creator_id = %s, name = %s, notes = %s, config_json = %s, start_timestamp = %s, end_timestamp = %s where id=%s;',
                (
                    creator_id,
                    name,
                    notes,
        ...
        # create new campaign
        cur.execute(
            'insert into et_campaigns(creator_id, name, notes, config_json, start_timestamp, end_timestamp) values (%s,%s,%s,%s,%s,%s) returning id;',
            (
                creator_id,
                name,
                notes,
...
def update_user_sync_timestamp(user_id):
    cur = conn.cursor()
    cur.execute(
        "update et_users set last_sync_timestamp = %s where id=%s;",
        (
            utils.get_timestamp_ms(),
            user_id
        )
...
def store_data_record(user_id, timestamp_ms, data_source_id, values):
    ...
    try:
        cur = conn.cursor()
        cur.execute(
            'insert into et_campaign_{0}_data(user_id, timestamp, data_source_id, values) values (%s,%s,%s,%s);'.format(campaign_id),
            (
                user_id,
                timestamp_ms,
                data_source_id,
        ...
        cur.close()
        update_user_sync_timestamp(user_id=user_id)
        conn.commit()
        return True
    except (Exception, psycopg2.DatabaseError) as e:
...
def get_participants_amount_of_data(user_id, campaign_id):
    cur = conn.cursor()
    cur.execute("select count(*) from et_campaign_{0}_data where user_id=%s;".format(campaign_id), (user_id,))
    row = cur.fetchone()
    cur.close()
    return row[0]


def get_amounts_of_data_separated_by_data_sources(user_id):
    res = []
    campaign_id = get_participants_current_campaign_id(user_id=user_id)
    ...
    for data_source in data_sources:
        cur.execute(
            'select count(*) from et_campaign_{0}_data where user_id=%s and where data_source_id=%s;'.format(campaign_id),
            (
                user_id,
                data_source[0]
            )
        )
        # res += [(data_source_id, amount_of_data)]
        res += [(data_source[0], cur.fetchone())]
    cur.close()
    return res
    ...
    if data_source_id == -1:
        if from_timestamp == -1:
            ...
            if more_data_available:
                cur.execute(
                    'select exists(select 1 from et_campaign_{0}_data where user_id=%s and timestamp>%s);'.format(campaign[0]),
                    (
                        user_id,
                        rows[0][1]
                    )
            ...
            cur.execute(
                'select * from et_campaign_{0}_data where user_id=%s and timestamp>%s order by timestamp limit(100);'.format(campaign[0]),
                (
                    user_id,
                    from_timestamp,
                )
        ...
        if from_timestamp == -1:
            cur.execute(
                'select * from (select * from et_campaign_{0}_data where user_id=%s and data_source_id=%s) as user_data order by timestamp limit(100);'.format(campaign[0]),
                (
                    user_id,
                    data_source_id
                )
            ...
            cur.execute(
                'select * from (select * from et_campaign_{0}_data where user_id=%s and timestamp>%s and data_source_id=%s) as user_data order by timestamp limit(100);'.format(campaign[0]),
                (
                    user_id,
                    from_timestamp,
                    data_source_id
...
def get_filtered_data(user_id, campaign, data_source_id, from_timestamp, till_timestamp):
    cur = conn.cursor()
    cur.execute(
        'select * from et_campaign_{0}_data where user_id=%s and data_source_id=%s and timestamp>%s and timestamp<%s order by timestamp;'.format(campaign[0]),
        (
            user_id,
            data_source_id,
            from_timestamp,
...
def create_data_source(creator_id, name, icon_name):
    cur = conn.cursor()
    cur.execute(
        "insert into et_data_sources(creator_id, name, icon_name) values (%s,%s,%s);",
        (
            creator_id,
            name,
            icon_name
...
def get_participants_amount_of_data_per_data_source(user_id, campaign):
    ...
    for config_json in configurations:
        ...
        cur.execute(
            'select count(*) from et_campaign_{0}_data where user_id=%s and data_source_id=%s;'.format(campaign[0]),
            (
                user_id,
                data_source_id
            )
        )
        amount_of_data = cur.fetchone()[0]
        res += [(data_source_id, amount_of_data)]
    cur.close()
    return res
